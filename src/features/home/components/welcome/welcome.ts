...
import { lucideSparkles, lucideX, lucideChevronLeft, lucideChevronRight } from '@ng-icons/lucide';
...
@Component({
  ...
  providers: [provideIcons({ lucideSparkles, lucideX, lucideChevronLeft, lucideChevronRight })],
            ...
            <div class="text-center mb-6">
              <div class="flex justify-center mb-4">
                <div class="icon-wrapper">
                  <ng-icon hlm size="lg" name="lucideSparkles" />
                </div>
              </div>
  ...
  styles: [`
    ...
    .icon-wrapper {
      ...
      height: 48px;
      border-radius: 1rem;
      background: linear-gradient(135deg, var(--primary) 0%, var(--primary-hover, #6366f1) 100%);
      display: flex;
      align-items: center;
      justify-content: center;
      color: white;
...
export class WelcomeComponent {
  ...
  prev() {
    this.currentIndex.update(i => (i - 1 + this.newsService.news().length) % this.newsService.news().length);
  }
